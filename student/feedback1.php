...
if (isset($_POST['btnSave'])) {
    ...
    $commentsArr = htmlentities(  $_POST['comment']);
    ...
    <head>
        ...
        <script>
            function x() {
                ...
                if (count < 14){
                    alert ('TCT-QA: "Please, Fill All Entries"');
                    return false;
                }
                else {
                    if (controls.comment.value.trim() == ''){
                        //alert ('TCT-QA: "Please, Leave Comment"');//By Dieudonne
						alert ('TCT-QA: "Please, Fill the Comment in the provided field"');//By Nepo

                        return false;
                    }
                    alert ('TCT-QA: "Feedback Submitted"');
                    return true;
                }
            }
        </script>
    ...
    <body>
        ...
        <div class="body">
            <div class="main_body">
                ...
                $str = listStudentSubjects($_SESSION['u_uname']);
                if ($str == "" && sizeof($course) < 1) {
                    if ($str == "") {
                    ...
                    if (firstOnThePage ()) {
                        ...
                        <p><font size = -1>To start evaluation process, click on any course to evaluate on the left side menu below the TCT Logo.</font></p>";
                        echo "<br /><p><font size = -1><strong>Quality Assurance Office</font></p>";
                    } else {
                        echo "<h3>Message from QA Office<h3>";
                        echo "<font size = -1>Choose the course to evaluate.</font>";
                    }
                } else if ($str == "" or $course == "") {
                    echo "<h3>Message from QA Office<h3>";
                    echo "<font size = -1>You have already evaluated this course</font>";
                } else {
                    $ret = plotFeedBackFormx($sid, $group, $course['subject_teacher_id'], $_SESSION['u_uname']);
                    if ($ret == "") {
                        echo '<h2>Access denied</h2><p><strong>Contact The System Administrator</strong></p>';
                    } else {
