...
{	
	...
	{	
		$selectQuery			=	"SELECT *  FROM `tbl_t_departments` WHERE `dpt_code` LIKE '{$deptCode}' OR `dpt_name` LIKE '{$deptName}'";
		$existFlag				=	recordAlreadyExist($selectQuery);
		if($existFlag)
		{
			$errorMsg			=	"Error! Departmet Already Exists!";
		}	
		else
		{	
			$deptInsertQuery	= 	"INSERT INTO `tbl_t_departments` (`dpt_id`, `dpt_code`, `dpt_name`) VALUES (NULL, '{$deptCode}', '{$deptName}')";
