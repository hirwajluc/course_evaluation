...
{
	$successMsg		=	"Subject Inforamtion Successfully Updated!";	
}
...
{	
	...
	{	
		$selectQuery	=	"SELECT *  FROM `tbl_subject` WHERE `subject_name` LIKE '{$subjectName}'
		AND `subject_ac_year` = '{$subjectAcYear}' AND `subject_code` LIKE '{$subjectCode}'
		AND `subject_year` LIKE '{$year}'
		AND `subject_group` = $group 
		AND `subject_semester` = $semester 
		AND `subject_department_id` = $departmentId 
		AND `subject_teacher_id` = $teacherId
		AND `subject_id` NOT LIKE {$subjectId}"; 
		...
		{	
			...
			$selectQuery	=	"SELECT *  FROM `tbl_subject` WHERE (`subject_name` LIKE '{$subjectName}' OR 
								...
								(subject_department_id = {$departmentId} AND 
								subject_teacher_id={$teacherId})  AND
			...
			{	
				...
				$subjectInsertQuery	=	"UPDATE `tbl_subject` SET `subject_ac_year` = '{$subjectAcYear}' , `subject_name` = '{$subjectName}' , `subject_code` = '{$subjectCode}',
										`subject_year` = '{$year}',`subject_group` = '{$group}',`subject_semester` = '{$semester}', `subject_department_id` = '{$departmentId}', `subject_teacher_id` = '{$teacherId}' WHERE `tbl_subject`.`subject_id` = {$subjectId} LIMIT 1"; 
...
<?php
	if(isset($_GET['id']))
	{
		...
		$answerSelQry	=	"SELECT tbl_subject.*, tbl_teacher.*
		FROM tbl_subject
		JOIN tbl_teacher ON tbl_subject.subject_teacher_id = tbl_teacher.teacher_id
		WHERE subject_id={$id}";
		...
		if($answerNumRows)
		{
			...
			$teacher_id					=	$answerSelArr['teacher_id'];
			$teacherDepartment			=	$answerSelArr['teacher_department_id'];
		}	
	}
?>
...
<div class="main">
	...
	<div class="body">
		<div class="main_body">
			<div>
				<a href="subjects_reuse.php?id=<?php echo $id;?>">Re-Use Subject</a>
			</div>
			<h2>Subject-Edit</h2>
			<?php
			...
			<form method = 'POST' action = "<?php echo "{$_SERVER['PHP_SELF']}?id={$_GET['id']}"?>">
				<table width ="550" border = '0' cellspacing = '0' cellpadding = '0'>
						...
						<td height = '30'>
							<div id='teacher_name_div'>
								<!-- <select name='teacher_id' id='teacher_id' class='typeproforms'>
									...
									<?php
									// $trSearchQry	=	"SELECT * FROM tbl_teacher ORDER BY teacher_first_name";
									// $trSearchRes	=	$connPDO->query($deptSearchQry);
									// $trNumRows	=	$deptSearchRes->rowCount();
									// if ($trNumRows > 0){
									// 	while ($trSearchArr	=	$trSearchRes->fetch(PDO::FETCH_ASSOC)){
									// 		if ($trSearchArr['teacher_id'] == $teacher_id){
									// 			?>
									// 			<option value="<?//=$trSearchArr['teacher_id'];?>" selected><?//=$trSearchArr['teacher_first_name'].' '.$trSearchArr['teacher_last_name'];?></option>
									// 			<?php
									// 		} else {
									// 			?>
									// 			<option value="<?//=$trSearchArr['teacher_id'];?>"><?//=$trSearchArr['teacher_first_name'].' '.$trSearchArr['teacher_last_name'];?></option>
									// 			<?php
									// 		}
									// 	}
									// }
					...
					<tr>
						<td>&nbsp;</td>
						<td height = '30'>
							<input type = 'submit' name = 'submit' class = 'button' value = 'Edit Subject' />
						</td>
					</tr>
				</table>
			</form><!-- End of form-->
