...
{
	$errorMsg		=	"Error!Unable to Update Program!";	
}
if(isset($_POST['submit']))
{
	...
	if($deptName	==	'')
	{
		$errorMsg	=	'Error! Program Name Should Not Be Left Blank!';	
	}	
	else
	{	
		$selectQuery			=	"SELECT *  FROM `tbl_department` WHERE `department_id` = {$deptId} AND `department_code` LIKE '{$deptCode}' AND `department_name` LIKE '{$deptName}'";
		$existFlag				=	recordAlreadyExist($selectQuery);
		if($existFlag)
		{
			$errorMsg			=	"Error! Program Name & Code Already Exist!";
		}	
		else
		{	
			$deptUpdateQuery	= 	"UPDATE `course_evaluation`.`tbl_department` SET `department_code` = '{$deptCode}', `department_name` = '{$deptName}' WHERE 
	...
	<div class="body">
		<div class="main_body">
			...
			<tr>
				<td>&nbsp;</td>
				<td height = '30'>
					<input type = 'submit' name = 'submit' class = 'button' value = 'Edit Program' />
				</td>
			</tr>
		  	</table>
			</form><!-- End of form-->
