...
{
	$successMsg		=	"Student Information Successfully Deleted!";	
}
if(isset($_GET['msg']) && ($_GET['msg'] == 'notdeleted'))
{
	$errorMsg		=	"Error! Unable to Delete Student Information!";	
}
    ...
    <div class="main_body">
      <h2>Student - Search</h2>
        ...
        <?php
          $keyword			=	trim($_GET['keyword']);
	   	  $searchQry		=	"SELECT *  FROM `tbl_users` INNER JOIN  tbl_department ON  
								tbl_users.stud_dept = tbl_department.department_id  
								WHERE (`u_fname` LIKE '{$keyword}%' OR `stud_regno` LIKE '{$keyword}%') AND u_utype='student' 
								ORDER BY u_id DESC";
		  ...
		  ?>
		      <tr>
				...
				<td height="30">&nbsp;
					<?php 
						echo $searchArr["stud_year"];
					?>
				</td>
				<td height="30">&nbsp;
					<?php 
						echo $searchArr["stud_sem"];
					?>
				</td>
                <td width="99" align="center">
					<a href="student_edit.php?id=<?php echo $searchArr["u_id"] ?>&keyword=<?php echo $_GET["keyword"]?>">Edit</a> | 
					<a onclick="javascript: return confirm('Sure! Do you want to Delete?');" href="student_delete.php?id=<?php echo $searchArr["u_id"]?>&keyword=
					<?php $_GET["keyword"]?>">Delete</a></td>
              </tr>
          <?php
		  		}
			}
      ...
      <p>&nbsp;</p>
		...
		<?php
			...
			echo plotLogoDiv();
			echo plotSearchDiv('student_search.php');
		?><!-- End of Search Div-->
    </div>	
		<div class="clr"></div>
		<br/><br/>
